...
import { ApiError, apiChangeMyPassword } from "@/src/api/client";
...
export default function ChangePasswordScreen() {
  ...
  const mutation = useMutation({
    mutationFn: () => apiChangeMyPassword(current, next),
    onSuccess: () => {
      setToast({ msg: t("password.saved"), ok: true });
      setTimeout(() => router.back(), 900);
    },
    onError: (e) => {
      if (e instanceof ApiError && e.status === 400) {
        setErrors((p) => ({ ...p, current: t("password.wrongCurrent") }));
        setToast({ msg: t("password.wrongCurrent"), ok: false });
      } else {
  ...
  const onSubmit = () => {
    ...
    if (next.length < 6) errs.next = t("password.minLength");
    if (confirm !== next) errs.confirm = t("password.mismatch");
    setErrors(errs);
    if (Object.keys(errs).length > 0) return;
    mutation.mutate();
  };

  const fields: { key: "current" | "next" | "confirm"; label: string; value: string; set: (v: string) => void }[] = [
    { key: "current", label: t("password.current"), value: current, set: setCurrent },
    { key: "next", label: t("password.new"), value: next, set: setNext },
    { key: "confirm", label: t("password.confirm"), value: confirm, set: setConfirm },
  ];
